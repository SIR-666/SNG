uint32_t step__ec(int flow, int sw_stat)
{
  ...
  */
  ...
  { currentmilis = millis();
    if (currentmilis - prevmilis > 50000)
    {
      NGstats = true;
      currentmilis = millis();
...
void run_SNG()
{
  ...
  if (FINISH == true)
  {
    ...
    {
      CUR_T_STEP = millis();
      if (CUR_T_STEP - PREV_T_STEP >= 50000 && NGstats == false)
      {
  ...
  if (MISSWIRE == true || MISSLOC == true || NGstats == true || NG_VT == true)
  {
